...
class Worley {
    constructor(setup = {}) {
        ...
        this.generateCrests(setup);
        this.hierarchy = setup.hierarchy;
        ...
        this.Texture = {
            parent: this,
            generate() {
                const data = new Uint8Array(this.parent.width * this.parent.height);
            ...
            ImageData(colors = this.parent.colors, alpha = this.parent.alpha, regenerate = false) {
                ...
                for (let i = 0; i < rgbaChanelData; i++) {
                    const scale = data[Math.floor(i / 4)] / 255;
                    ...
                    // Monochrome
                    if (!colors) {
                        imgData.data[i] = data[Math.floor(i / 4)];
                        continue;
                    };
    ...
    addCrest(x, y, relative = false) {
        ...
        if (x == undefined || y == undefined) throw new Error("Cannot add crest without given coordinates.");

        // Check if co-ordinates are relative
        this.crests.push(relative ? [Math.round(x * this.width), Math.round(y * this.height)] : [x, y]);
    };

    nearestCrest(x = 0, y = 0, hierarchy = 0) {
        if (this.crests.length === 0) return null;
        ...
        const iterations = crests.length;
        ...
        // Find nearest crest
        return Worley.find(crests, hierarchy).crest;
    };

    pixel(x, y, interpolate = this.interpolate, hierarchy = this.hierarchy) {
        x++;
        let nearestCrest = this.nearestCrest(x, y, hierarchy);
    ...
    generateCrests(setup) {
        ...
        const prerun = setup.prerun;
        const crests = setup.crests;
        for (let i = 0; i <= prerun; i++) rand();

        // Generate the crests
        for (let i = 0; i < crests; i++) this.addCrest(rand(), rand(), true);
    };
